...
async function getRemoteDependencies(root: string) {
  ...
  function getDeps(graph: ModuleGraph, specifier: string) {
    ...
    if (dependencies) {
      for (const { code, type } of dependencies) {
        ...
        if (new URL(code.specifier).protocol === "file:") {
          ...
          getDeps(graph, code?.specifier ?? type?.specifier!);
        } else {
...
export async function importUrls(targetFile: string) {
